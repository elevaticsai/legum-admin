...
export const ComplianceOverview: React.FC<ComplianceOverviewProps> = ({ data }) => {
  const calculateStats = () => {
    ...
    const stats = {
      ...
      abstract: { total: 0, compliant: 0 }, // Changed from announcement to abstract
    ...
    Object.entries(data).forEach(([key, items]) => {
      if (key !== 'employee') {
        ...
        // Map 'announcement' to 'abstract' in the stats
  ...
  const donutData = [
    ...
    { name: 'Non-Compliant', value: total - compliant, color: '#ef4444' }
  ];

  const getCategoryLabel = (key: string) => {
    switch (key) {
      case 'license': return 'License & Registration';
      case 'abstract': return 'Abstract & Notices';
      case 'register': return 'Register & Records';
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-4 sm:p-6 mb-6">
      <h2 className="text-lg font-semibold mb-4">Overall Compliance Status</h2>
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-9 grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
          {Object.entries(stats).map(([key, value]) => {
            const categoryRate = value.total > 0 ? Math.round((value.compliant / value.total) * 100) : 0;
            return (
              <div key={key} className="space-y-2">
                <div className="flex justify-between items-center">
                  <div className="flex items-center space-x-2">
                    {key === 'license' && <FileText className="w-4 h-4 text-blue-600" />}
                    {key === 'abstract' && <AlertTriangle className="w-4 h-4 text-yellow-600" />}
                    {key === 'register' && <CheckCircle className="w-4 h-4 text-green-600" />}
                    {key === 'return' && <Clock className="w-4 h-4 text-purple-600" />}
                    {key === 'remittance' && <FileText className="w-4 h-4 text-indigo-600" />}
                ...
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div
                    className={`h-2 rounded-full ${
                      categoryRate >= 90 ? 'bg-green-600' :
                      categoryRate >= 70 ? 'bg-yellow-600' : 'bg-red-600'
                    }`}
                    style={{ width: `${categoryRate}%` }}
                  />
                </div>
                <div className="flex justify-between text-xs text-gray-500">
                  <span>{value.compliant} Compliant</span>
                  <span>{value.total - value.compliant} Non-Compliant</span>
                </div>
              </div>
            );
          })}
